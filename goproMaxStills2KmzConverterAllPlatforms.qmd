...
This workflow creates a .kmz file from geocoded 360° images taken with a GoPro Max. It involves package setup, file renaming, selecting images by distance (20m default), adding overlays (optional), creating a .kml file, then finally converting everything to a .kmz file for use in the [pannotator](https://cran.r-project.org/web/packages/pannotator/index.html) package for annotating. To use this script open the pannotator_collect.Rproj in RStudio, then open this file (goproMaxStills2KmzConverter.qmd) and [**run each code chunk in order using the play arrow at the top-right corner of the chunk**]{.smallcaps} to create a .kmz file.
...
# (Windows) use forward slashes only, no spaces in folder or file names 
...
# 
# # Set to TRUE to add overlays to each image file; or FALSE to use images without any overlays.
# addOverlays <- svDialogs::dlg_input(message = "Add Overlays to images?", default = "FALSE")$res
# 
# # Conditionally set the overlay image file if addOverlays is TRUE
# if (addOverlays == TRUE) {
