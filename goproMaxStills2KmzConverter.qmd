...
This workflow creates a .kmz file from geocoded 360° images taken with a GoPro Max. It involves package setup, file renaming, selecting images by distance (20m default), adding overlays (optional), creating a .kml file, then finally converting everything to a .kmz file for use in the [pannotator](https://cran.r-project.org/web/packages/pannotator/index.html) package for annotating. To use this script open the pannotator_collect.Rproj in RStudio, then open this file (goproMaxStills2 KmzConverter.qmd) and run each code chunk in order to create a .kmz file.
