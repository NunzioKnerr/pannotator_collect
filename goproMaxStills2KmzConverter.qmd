...
In RStudio use the little green arrow at the top-right corner of the chunk to run them.
...
Here we set the user options which will be used in the following code chunks

```{r eval=FALSE, echo=TRUE, warning=FALSE, include=TRUE}

svDialogs::msgBox("This is a message to test")

# Choose the directory path containing 360 images.
directory <- svDialogs::dlg_dir(default = getwd(), title = "Select Directory Containing 360 images", gui = .GUI)$res
# OR
# add path manually: use forward slashed only.
# directory <- "C:/FolderContaining360Images/"

# Select the minimum distance in metres between each extracted image.
# For example, set metresBetweenEachImageWanted <- 100 to extract images a minimum of 100 metre apart.

metresBetweenEachImageWanted <- svDialogs::dlg_input(message = "Enter a value for: metresBetweenEachImageWanted", default = "100", gui = .GUI)$res
# OR
...
addOverlays <- svDialogs::dlgList(choices=c(TRUE,FALSE), preselect = TRUE, multiple = FALSE, title = "Add overlay to images?", gui = .GUI)$res
# OR
#addOverlays <- TRUE
...
if (addOverlays == TRUE) {
  # Choose the file for your desired overlay image (PNG image with transparency).
   overlayImageFile <- svDialogs::dlg_open(default = "./overlay_files",title = "Select overlay file (png)", multiple = FALSE, filters = svDialogs::dlg_filters["png", ], gui = .GUI)$res
  # OR
  # specify file path manually.
  # overlayImageFile <- "overlay_files/5m_overlay_wedges_straight6.png"
}

#svDialogs::dlg_message(message ="To have selected XXX", type = c("ok"),gui = .GUI)
