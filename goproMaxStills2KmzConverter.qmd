...
In order for this workflow to function as expected there are a few dependent packages to install and configure. If you are on linux you may have some issues with the 'magick' package, check the documentation [here](https://cran.r-project.org/web/packages/magick/vignettes/intro.html).
...
directory <- rstudioapi::selectDirectory(caption = "Select Directory Containing 360 images", label = "Select", path = rstudioapi::getActiveProject())
# OR
...
if (addOverlays) {
  ...
  overlayImageFile <- rstudioapi::selectFile(caption = "Select overlay file (png)", label = "Select PNG file", path = rstudioapi::getActiveProject(), filter = "png (*.png)",)
  # OR
